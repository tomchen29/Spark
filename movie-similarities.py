...
data = SparkContext(conf = SparkConf().setMaster("local[*]").setAppName("MovieSimilarities")).textFile("ml-100k/u.data")
...
if __name__ == "__main__":
    ...
    results = results.sortByKey(ascending = False).take(10000)

    nameDict = {}
    with open("ml-100k/u.item") as f:
        for line in f:
            fields = line.split('|')
            nameDict[int(fields[0])] = fields[1]
